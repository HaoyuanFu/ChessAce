...
\noindent checkmate(c, A):
\begin{itemize}
\item output: Check if all move in King's posMove list are valid, and check if any friendly piece can eliminate the piece that cause the stalemate.\\
\\
$out := \forall(i, k : Cell, King | i \in retrieveKing(c).posMove() : move(retrieveKing(c).Cell, i)\\ \Rightarrow i.King.isKingInDanger(pos))\\ \wedge \\ \neg (\exists(p : Piece | p.getColor() = retrieveKing(c).getColor(): A \in p.posMove()))$
\item exception: none
\end{itemize}
